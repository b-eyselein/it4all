...
class ReplacementTest {
  ...
  @Test
  def testSequence() {
    testReplacement(aRepl ~ aRepl, "A , A", List(List(aRepl, aRepl)))
  }

  @Test
  def testAlternative() {
    testReplacement(aRepl | bRepl, "A | B", List(List(aRepl), List(bRepl)))
  }

  @Test
  def testOption() {
    testUnaryReplacement(aRepl.?, aRepl, a.toString + "?", List(List.empty, List(aRepl)))
  }

  @Test
  def testRepetition() {
    testUnaryReplacement(aRepl.*, aRepl, a.toString + "*",
      List(List.empty, List(aRepl), List(aRepl, aRepl), List(aRepl, aRepl, aRepl)))
  }

  @Test
  def testMin1Repetition() {
    testUnaryReplacement(aRepl.+, aRepl, a.toString + "+",
  ...
  @Test
  def testGrouping() {
    testReplacement(Grouping(aRepl), "(A)", List.empty)
  }

  @Test
  def testTerminal() {
    testSymbolReplacement(TerminalReplacement(one), one, "'1'", List.empty)
  }

  @Test
  def testVariable() {
    testSymbolReplacement(aRepl, a, "A", List.empty)
  }
