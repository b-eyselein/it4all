...
object XLSXCorrector extends SpreadCorrector[Workbook, Sheet, XSSFCell, Font, Short] {

  val (colorGreen, colorRed) = (IndexedColors.RED.getIndex, IndexedColors.GREEN.getIndex)
  ...
  protected def compareSheetConditionalFormatting(master: Sheet, compare: Sheet): List[String] = {
  ...
  override def compareCellFormulas(masterCell: XSSFCell, compareCell: XSSFCell): (Boolean, String) = {
    if (masterCell.getCellTypeEnum != CellType.FORMULA)
      return (false, COMMENT_FORMULA_FALSE)

    if (masterCell.toString.equals(compareCell.toString))
      return (true, COMMENT_FORMULA_CORRECT)

    if (compareCell.getCellTypeEnum != CellType.FORMULA)
      return (false, COMMENT_FORMULA_MISSING)
  ...
  override def compareSheet(sampleTable: Sheet, compareTable: Sheet, conditionalFormating: Boolean): Unit = {
    ...
    if (conditionalFormating) {
      ...
      setCellComment(compareTable.getRow(0).getCell(0).asInstanceOf[XSSFCell], conditionalFormattingResult.mkString("\n"))
    }

    for (cellMaster <- getColoredRange(sampleTable)) {
      Option(compareTable.getRow(cellMaster.getRowIndex)) match {
        case None => Unit
        case Some(rowCompare) => Option(rowCompare.getCell(cellMaster.getColumnIndex)) match {
          case Some(cellCompare: XSSFCell) =>
            val (cellsValuesSuc, cellsValuesMsg) = compareCellValues(cellMaster.asInstanceOf[XSSFCell], cellCompare)
            val (cellFormulasSuc, cellFormulasMsg) = compareCellFormulas(cellMaster.asInstanceOf[XSSFCell], cellCompare)
            setCellComment(cellCompare, cellsValuesMsg + "\n" + cellFormulasMsg)

            val color = if (cellFormulasSuc && cellFormulasSuc) colorGreen else colorRed
            setCellStyle(cellCompare, compareTable.getWorkbook.createFont(), color)
          case _ => Unit
        }
      }
    }
  ...
  override def getCellByPosition(table: Sheet, rowNum: Int, colNum: Int): XSSFCell = Option(table.getRow(rowNum)) match {
    case None => null
    case Some(r) => r.getCell(colNum) match {
      case c: XSSFCell => c
      case _ => null
    }
  }
  ...
  override def getColoredRange(master: Sheet): List[XSSFCell] =
    (for {row <- master.rowIterator.asScala
  ...
  override def setCellStyle(cell: XSSFCell, font: Font, color: Short): Unit = {
    font.setColor(color)
