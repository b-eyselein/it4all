...
abstract class SpreadCorrector[D, S, C, F, ColorType] {
  ...
  def correct(samplePath: Path, comparePath: Path, conditionalFormating: Boolean, compareCharts: Boolean): SpreadSheetCorrectionResult = {
    if (!samplePath.toFile.exists) SpreadSheetCorrectionFailure(ERROR_MISSING_SAMPLE)
      ...
      case Failure(e) => SpreadSheetCorrectionFailure(ERROR_LOAD_SAMPLE)

      case Success(sampleDocument) => loadDocument(comparePath) match {

        case Failure(e) => SpreadSheetCorrectionFailure(ERROR_LOAD_SOLUTION)
        ...
        case Success(compareDocument) =>
          if (getSheetCount(sampleDocument) != getSheetCount(compareDocument))
            ...
            if (compareCharts) {
              val (chartCompSuc, chartCompMessage) = compareNumberOfChartsInDocument(compareDocument, sampleDocument)
              // write message in Cell A0 on first Sheet
              setCellComment(getCellByPosition(getSheetByIndex(compareDocument, 0), 0, 0), chartCompMessage)
            }
  ...
  protected def getCellByPosition(table: S, row: Int, column: Int): C
  ...
  protected def setCellStyle(cell: C, font: F, color: ColorType): Unit
