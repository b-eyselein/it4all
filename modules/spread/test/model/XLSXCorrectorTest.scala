...
class XLSXCorrectorTest {
  ...
  @Test
  def testCompareCellFormulas = XLSXCorrector.loadDocument(testMusterCopy).zip(XLSXCorrector.loadDocument(testSolution)) match {
    case Failure(e) => Assert.fail(failureLoad(null, e))
    case Success((muster, teilLsg)) =>
      XLSXCorrector.compareCellFormulas(getCell(muster, 2, 15, 7), getCell(teilLsg, 2, 15, 7)).shouldBe(COMMENT_FORMULA_CORRECT)

      // Kein Formel in Muster, Compare leer
      XLSXCorrector.compareCellFormulas(getCell(muster, 3, 16, 1), getCell(teilLsg, 3, 16, 1)).shouldBe(COMMENT_FORMULA_FALSE)

      // Wert in Muster, Compare leer
      XLSXCorrector.compareCellFormulas(getCell(muster, 3, 16, 5), getCell(teilLsg, 3, 16, 5)).shouldBe(COMMENT_FORMULA_MISSING)

      // Wert in Muster, Compare leer
      XLSXCorrector.compareCellFormulas(getCell(muster, 3, 19, 5), getCell(teilLsg, 3, 19, 5)).shouldBe("Formel falsch. Der Bereich [D20] fehlt.")
  }

  @Test
  def testCompareCellValues() = XLSXCorrector.loadDocument(testMusterCopy).zip(XLSXCorrector.loadDocument(testSolution)) match {
    case Failure(e) => Assert.fail(failureLoad(null, e))
    case Success((muster, teilLsg)) =>

      XLSXCorrector.compareCellValues(getCell(muster, 2, 15, 7), getCell(teilLsg, 2, 15, 7)).shouldBe(COMMENT_VALUE_CORRECT)

      // Wert in Muster, Compare leer
      XLSXCorrector.compareCellValues(getCell(muster, 3, 16, 5), getCell(teilLsg, 3, 16, 5)).shouldBe(COMMENT_VALUE_MISSING)

      // Wert in Muster, Compare falsch
      // TODO: Zahl schoener formatieren... 12.142857142857142
      XLSXCorrector.compareCellValues(getCell(muster, 3, 19, 5), getCell(teilLsg, 3, 19, 5)).shouldBe(String.format(COMMENT_VALUE_INCORRECT_VAR, 12.142857142857142))
  }

  @Test
  def testCompareChartsInSheet() = XLSXCorrector.loadDocument(testMusterCopy).zip(XLSXCorrector.loadDocument(testSolution)) match {
    case Failure(e) => Assert.fail(failureLoad(null, e))
    case Success((muster, teilLsg)) =>
      XLSXCorrector.compareChartsInSheet(muster.getSheetAt(0), muster.getSheetAt(0)).shouldBe(COMMENT_CHART_FALSE)

      XLSXCorrector.compareChartsInSheet(teilLsg.getSheetAt(0), muster.getSheetAt(2)).shouldBe(String.format(COMMENT_CHART_NUM_INCORRECT_VAR, 2, 0))

      XLSXCorrector.compareChartsInSheet(muster.getSheetAt(2), muster.getSheetAt(2)).shouldBe(COMMENT_CHART_CORRECT)
    // TODO: Implement!
  ...
  def testCompareNumberOfChartsInDocument(): Unit = XLSXCorrector.loadDocument(stdDoc) match {
    ...
    case Success(document) => assertNull(XLSXCorrector.compareNumberOfChartsInDocument(document, document))
  }

  @Test
  def testCompareSheet() {
  ...
  @Test
  def testGetColoredRange() = XLSXCorrector.loadDocument(testMusterCopy) match {
    case Failure(e) => Assert.fail(failureLoad(testMusterCopy, e))
    case Success(muster) =>
