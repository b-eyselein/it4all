...
public class NameTask extends Task {
  ...
  public ElementResult<? extends Task> evaluate(SearchContext searchContext) {
    ...
    if(foundElements.isEmpty())
      return new NameResult(this, Success.NONE, Collections.emptyList());
    ...
    // childResults.parallelStream().forEach(childRes ->
    // childRes.setSuccess(childRes.evaluate(element)));

    List<AttributeResult> attributeResults = checkAttributes(element);

    if(allAttributesFound(attributeResults))
      return new NameResult(this, Success.COMPLETE, attributeResults);
    else
      return new NameResult(this, Success.PARTIALLY, attributeResults);
  }
}
