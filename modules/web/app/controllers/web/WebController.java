...
public class WebController extends ExerciseController<WebExercise, WebResult> {
  ...
  public Result correct(int id, String type) {
    ...
    CompleteResult<WebResult> result = correct(learnerSolution, exercise, user, isJS);
    log(user, new ExerciseCompletionEvent(request(), id, result));

    return ok(
        views.html.correction.render("Web", renderResult(result), learnerSolution, user, routes.WebController.index()));
  }
