...
public class Util {
	...
	/**
	 * This method creates parent-child tuple for all trees.
	 * Also all tuples corresponding to the same tree are put in a HashMap.
	 * Key is the text of the root.
	 * NOTE: as in the definition of TreeNode demanded, the text of a TreeNode is unique
	 *     A
	 *    / \
	 *   B   C 
	 *  / \
	 * D   E
	 * Tuples are: (A,B), (A,C), (B,D), (B,E)
	 * 
	 * @param listOfRoots roots of trees
	 * @return HashMap which contains all tuples of a tree for all trees. Key is the text of the root of the tree.
	 */
	public static HashMap<String, LinkedList<Tuple>> buildTuples(LinkedList<TreeNode> listOfRoots) {
		HashMap<String, LinkedList<Tuple>> map = new HashMap<String, LinkedList<Tuple>>();
		for(TreeNode root : listOfRoots) {
			LinkedList<Tuple> listOfTuple = new LinkedList<Tuple>();
	...
	private static void buildTuple(LinkedList<Tuple> listOfTupel, TreeNode node) {
	 ...
	 * is calculated.
	 * 
	 * @param solutionRoots List of root nodes from the solution file
	 * @param inputRoots list of root nodes from the input file
	 */
	public static void applyMetaDataFromSolutionToInput(LinkedList<TreeNode> solutionRoots,
			LinkedList<TreeNode> inputRoots) {
	...
	private static void traverseTreeAndSetMetaData(TreeNode treeNode, LinkedList<TreeNode> inputRoots) {
	 ...
	 * the corresponding tree of the user input file contains these tuples.
	 * For each tuple possible outcomes are:
	 * - The user input tree contains the tuple: everything is correct
	 * - The user input tree does not contain the tuple: tuple is saved as missing
	 * - The user input tree does contain a tuple but the
	 *   text of the nodes of the tuple is slightly wrong: tuple is saved as partially correct
	 * - The user input contains a tuple which is not in the solution: tuple is saved as wrong
	 * This is done for all trees.
	 * 
	 * @param solutions Map of all trees with root.text as keys and 
	 * 					all tuples of the specific tree 
	 * @param input Map of all trees with root.text as keys and 
	 * 				all tuples of the specific tree
	 * @return Container class which holds all wrong/missing/partially correct tuples for each tree
	 */
	public static TupelDifferenceContainer getDifferenceTupel( 
			HashMap<String, LinkedList<Tuple>> solutions,
			HashMap<String, LinkedList<Tuple>> input) {
		HashMap<String, LinkedList<Tuple>> mapMissingTuple = new HashMap<String, LinkedList<Tuple>>();
		HashMap<String, LinkedList<Tuple>> mapWrongTuple = new HashMap<String, LinkedList<Tuple>>();
		HashMap<String, LinkedList<Tuple>> mapPartiallyCorrect = new HashMap<String, LinkedList<Tuple>>();
		for(String key : solutions.keySet()) {
			//tmpList contains all tuple which differ from the exact solution but are fine by the definition of synonyms
			//this is a list because of patterns like .*
			LinkedList<Tuple> tmpList;
			LinkedList<Tuple> solutionValues = solutions.get(key);
			//all roots in solution file MUST exist as roots in received input file
			//TODO allow this to happen
			//Problem: if root is different i don't know which solution tree maps to the input tree 
			LinkedList<Tuple> inputValues = input.get(key);
			LinkedList<Tuple> missing = new LinkedList<Tuple>();
			LinkedList<Tuple> existing = new LinkedList<Tuple>();
			LinkedList<Tuple> partiallyCorrectList = new LinkedList<Tuple>();
			for(Tuple solTupel : solutionValues) {
				boolean contains = false;
				boolean partiallyCorrect = false;
				tmpList = new LinkedList<Tuple>();
				//check for identity and synonyms identity including regular expressions
		...
		TupelDifferenceContainer result = new TupelDifferenceContainer(mapMissingTuple,
				mapWrongTuple, mapPartiallyCorrect);
		return result;
	}
	...
	private static boolean isPartiallyCorrect(Tuple solTupel, Tuple inTupel) {
		boolean firstPartiallyCorrect = 
				Levenshtein.isPartiallyCorrect(solTupel.getFirstNode(), inTupel.getFirstNode());
		boolean secondPartiallyCorrect = 
				Levenshtein.isPartiallyCorrect(solTupel.getSecondNode(), inTupel.getSecondNode());
		...
		//if both are partially correct or one is partially and one completely correct (identity and regex) return true
		//both completely correct is already impossible because this case was already handled 
	...
	private static void addDivergentSolutions(Tuple solTupel, LinkedList<Tuple> listToCheck,
			LinkedList<Tuple> resultingList) {
	...
	/**
	 * Adds to every node in the trees their respective state depending on
	 * the results of @see {@link #getDifferenceTupel(HashMap, HashMap)}} 
	 * 
	 * @param rootList List of roots
	 * @param result assignment of tuples to missing/wrong/partially correct 
	 */
	public static void applyResultToNodes(LinkedList<TreeNode> rootList, TupelDifferenceContainer result) {
		for(TreeNode root : rootList) {
			LinkedList<Tuple> missingTuple = result.getMapMissing().get(root.getText());
			LinkedList<Tuple> wrongTuple = result.getMapWrong().get(root.getText());
			LinkedList<Tuple> partiallyCorrectTuple = result.getMapPartiallyCorrect().get(root.getText());
			traverseTreeAndSetResults(root, missingTuple, wrongTuple, partiallyCorrectTuple);
		}	
	}
	
	private static void traverseTreeAndSetResults(TreeNode treeNode, LinkedList<Tuple> missingTuple,
			LinkedList<Tuple> wrongTuple, LinkedList<Tuple> partiallyCorrectTuple) {
	 ...
	 * 
	 * @param solutionRoots roots of solution map
	 * @param inputRoots roots of user input map
	 */
	public static void checkIfFeedbackPossible(LinkedList<TreeNode> solutionRoots,
			LinkedList<TreeNode> inputRoots) {
		for(TreeNode rootInput : inputRoots) {
	...
	private static void traverseForFeedback(LinkedList<TreeNode> solutionRoots, TreeNode inputNode) {
	...
	public int getMaxDepthOfTrees(LinkedList<TreeNode> rootList) {
		int maxDepth = 0;
		for(TreeNode root : rootList) {
			treeDepth = -1;
	...
	private void traverseTreeForMaxDepth(TreeNode treeNode, int currentDepth) {
		...
		currentDepth++;
		for(TreeNode child : treeNode.getChildren()) {
			traverseTreeForMaxDepth(child, currentDepth);
		}
	}
	
	/**
	 * calculate the achieved points depending on the the results of 
	 * @see {@link #getDifferenceTupel(HashMap, HashMap)}}.
	 * Currently wrong nodes are rated with zero points and for partially correct
	 * nodes the rating is based on the percentage difference 
	 * @see {@link #applyMetaDataFromSolutionToInput(LinkedList, LinkedList)}} 
	 * and the maximal points. 
	 * 
	 * @param rootList
	 */
	public static void setRealPoints(LinkedList<TreeNode> rootList) {
		for(TreeNode root : rootList) {
	 ...
	 * 
	 * @param rootList roots of the trees.
	 * @return Container class which holds the result and the maximal points
	 */
	public static PointsResult getOverallPoints(LinkedList<TreeNode> rootList) {
	...
	/**
	 * Inserts a dummy node as root node of all roots of the given list.
	 * This makes one tree out of many. This is used for TOC.
	 * 
	 * @param rootList roots of trees
	 * @return list of roots but only with one root
	 */
	public static LinkedList<TreeNode> mergeTrees(LinkedList<TreeNode> rootList) {
		LinkedList<TreeNode> noRoots = new LinkedList<TreeNode>();
		for(TreeNode root : rootList) {
			if(root.isMetaNode()) {
				noRoots.add(root);				
			}
		...
		if(rootList.size() > 1) {
			...
			LinkedList<TreeNode> onlyOneRoot = new LinkedList<TreeNode>();
