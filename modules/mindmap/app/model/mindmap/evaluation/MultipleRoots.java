...
public class MultipleRoots {
	...
	public void evalMultiRootMapWithExcel(String parserType, String input,
			String solution, String result, String alteredSolution,
			String alteredInput, String metaData, String template) throws Exception {
		RWExcel rwe = new RWExcel();
		//must be uncommented to be a console program - else mds and modus are set from outside
...
//		modus = askUserForModus();
		...
		LinkedList<TreeNode> inputRoots = abstractEvaluationParser.read(new File(input));
		LinkedList<TreeNode> solutionRoots = abstractEvaluationParser.read(new File(solution));	
		//this must be called before handleMetaData()...            //else there might be redundant 'yes' in the meta file. (example: press = 1, Haustier = yes, Dackel = yes)
		...
		Util.applyMetaDataFromSolutionToInput(solutionRoots, inputRoots);
		HashMap<String, LinkedList<Tuple>> mapInput = Util.buildTuples(inputRoots);
		HashMap<String, LinkedList<Tuple>> mapSolutions = Util.buildTuples(solutionRoots);
	...
	public void createMetaFromMap(String parserType, String solutionMapPath, String metaToCreatePath) throws Exception {
		RWExcel rwe = new RWExcel();
		AbstractEvaluationParser abstractEvaluationParser = ParserFactory.getEvaluationParser(parserType);
		LinkedList<TreeNode> solutionRoots = abstractEvaluationParser.read(new File(solutionMapPath));	
...
//		System.out.println("Press 1: Read from MindMap (reminder: for this your have to change the solution path; see main() method)");
...
//		System.out.println("Press 1: Minimal modus (only colored results and points)");
//		System.out.println("Press 2: Maximal modus (all available informations are displayed)");
//		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
//		int in = -1;
//		try {
//			in = Integer.parseInt(br.readLine());
