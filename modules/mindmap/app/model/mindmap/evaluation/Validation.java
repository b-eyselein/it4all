...
public class Validation {
	 ...
	 * 
	 * @param mapPath file path to mindmap
	...
	/**
	 * Validates the meta file which is given as file path. Even so this validation
	 * was true, there might occur errors at evaluation.
	 * 
	 * @param metaPath file path to meta file
	 * @return true if validation was correct, else false
	 */
	 ...
	 * method returns false. To build the new meta file, the solution file is needed.
	 * 
	 * @param parserType declares which parser will be used
	 * @param solutionPath file path to the perfect solution mindmap
	 * @param metaFilePath file path where the meta file is or should be
	 * @return true if the meta file does already exist. Else false and
	...
	public static boolean checkForMeta(String parserType, String solutionPath, String metaFilePath) throws Exception {
		if(new File(metaFilePath).exists()) {
			...
			LinkedList<TreeNode> solutionRoots = (ParserFactory.getEvaluationParser(parserType)
					).read(new File(solutionPath));
	...
	/**
	 * The MindManager parser is used. Else @see {@link #checkForMeta(String, String, String)}}
	 * 
	 * @param solutionPath
	 * @param metaFilePath
	 * @return
	...
	public static boolean checkForMeta(String solutionPath, String metaFilePath) throws Exception {
		if(new File(metaFilePath).exists()) {
			...
			LinkedList<TreeNode> solutionRoots = (ParserFactory.getEvaluationParser("MINDMANAGER")
					).read(new File(solutionPath));
			Util.applyMetaDataFromSolutionToInput(solutionRoots, new LinkedList<>());
			(new RWExcel()).createEmptyMetaFile(metaFilePath, solutionRoots);
			return false;
