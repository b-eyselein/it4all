...
public abstract class ProgLangCorrector {
  ...
  public CompleteResult correct(ProgrammingExercise exercise, String learnerSolution, List<ITestData> userTestData,
    ...
    List<EvaluationResult> results = Stream.concat(exercise.functiontests.stream(), userTestData.stream())
        .map(test -> evaluate(exercise, test, engine)).collect(Collectors.toList());

    return new CompleteResult(learnerSolution, results, todo);
  }

  protected EvaluationResult evaluate(ProgrammingExercise exercise, ITestData testData, ScriptEngine engine) {
  ...
  protected abstract EvaluationResult validateResult(ProgrammingExercise exercise, ITestData testData,
      String toEvaluate, Object realResult, Object awaitedResult, String output);
