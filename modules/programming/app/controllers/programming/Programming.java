...
public class Programming extends ExerciseController {
  ...
  public Result testData(int id) {
    User user = getUser();
    // @formatter:off
    List<CommitedTestData> oldTestData = Arrays.asList(
        new CommitedTestData(user.name, id, 0, "1#2", "3"),
        new CommitedTestData(user.name, id, 1, "4#5", "6"),
        new CommitedTestData(user.name, id, 2, "7#8", "15")
    );
    //@formatter:on
