...
public class BoolFormelParser {
	...
	public BoolscheFormelTree getBFTree(String originalformel) {
		...
		zeichensatz.addAll( Arrays.asList( '0', '1', '(', ')', ' ', 'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z' ));
		for (int i = 0; i < formel.length(); i++) {
		...
		for (int i = 0; i < ausdruck.length()-2; i++) {
			...
			if (klammer == 0) {
				if (ausdruck.substring(i, i+3).equals("xor") ) {
					return new BF_XOR(getNextKnoten(ausdruck.substring(0, i), vars), getNextKnoten(ausdruck.substring(i+3, ausdruck.length()), vars));
				}
			}
		}
		// suche or
		...
		for (int i = 0; i < ausdruck.length()-1; i++) {
			...
			if (klammer == 0) {
				if (ausdruck.substring(i, i+2).equals("or") ) {
					return new BF_OR(getNextKnoten(ausdruck.substring(0, i), vars), getNextKnoten(ausdruck.substring(i+2, ausdruck.length()), vars));
				}
			}
		}
		// suche and
		...
		for (int i = 0; i < ausdruck.length()-2; i++) {
			...
			if (klammer == 0) {
				if (ausdruck.substring(i, i+3).equals("and") ) {
					return new BF_AND(getNextKnoten(ausdruck.substring(0, i), vars), getNextKnoten(ausdruck.substring(i+3, ausdruck.length()), vars));
				}
			}
		}
		// suche not
