...
class ApiAdminController @Inject()(
  ...
  def readLessons(toolId: String): Action[AnyContent] = JwtAuthenticatedToolMainAction(toolId).async { implicit request =>
    ws.url(s"$resourcesServerBaseUrl/${request.toolMain.urlPart}/lessons")
      .get()
      .map { request => (request.json \ "lessons").validate[Seq[Lesson]](Reads.seq(lf)) }
      .map {
