...
abstract class SqlExecutionDAO(mainDbName: String) {
  ...
  def executeQueries(scenario: ExerciseCollection, exercise: SqlExerciseContent, userStatement: Statement, sampleStatement: Statement): SqlExecutionResult = {
  ...
  override protected def executeQuery(schemaName: String, query: Statement): Try[SqlQueryResult] = query match {
    case sel: Select =>
      using(db(Some(schemaName)).source.createConnection()) { connection =>
        using(connection.prepareStatement(sel.toString)) { statement => SqlQueryResult.fromResultSet(statement.executeQuery()) }
      }.flatten
    case _           => Failure(null)
  }
}
    ...
    case change@(_: Update | _: Insert | _: Delete) =>
      using(db(Some(schemaName)).source.createConnection()) { connection =>
        ...
        using(connection.prepareStatement(change.toString)) { statement => statement.executeUpdate() } flatMap { _ =>
          ...
          val res = using(connection.prepareStatement(validationQuery)) { statement => SqlQueryResult.fromResultSet(statement.executeQuery()) }
