...
function htmlForClassEdit(cellView) {
    let umlClass: UmlClass = cellView.model.getAsUmlClass();
...
function classMemberFromHtml(memberGroup): ClassMemberAttrs {
    return {
        visibilityRepr: $(memberGroup).find('select[data-for="visibility"]').val() as string,
        name: $(memberGroup).find('input[data-for="name"]').val() as string,
        type: $(memberGroup).find('select[data-for="type"]').val() as string,
    }
}

function umlClassAttributeFromHtml(memberGroup): UmlClassAttribute {
...
function umlClassMethodFromHtml(memberGroup): UmlClassMethod {
    let base: ClassMemberAttrs = classMemberFromHtml(memberGroup);
