...
object RegexDbModels extends DbModels {
  ...
  override def dbExerciseFromExercise(ex: RegexExercise): DbRegexExercise =
    DbRegexExercise(ex.id, ex.semanticVersion, ex.title, ex.author, ex.text, ex.state)
...
final case class DbRegexExercise(id: Int, semanticVersion: SemanticVersion, title: String, author: String, text: String,
                                 state: ExerciseState) extends HasBaseValues
