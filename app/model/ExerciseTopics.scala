...
trait ExerciseTopicsRepository {
  ...
  protected class ExerciseTopicsTable(tag: Tag) extends Table[ExerciseTopicRow](tag, "exercise_topics") {
    ...
    @unused def pk = primaryKey("exercise_topics_pk", (toolId, collectionId, exerciseId, topicAbbreviation))

    @unused def exerciseForeignKey = foreignKey("exercise_topics_exercise_fk", (toolId, collectionId, exerciseId), exercisesTQ)(
      ex => (ex.toolId, ex.collectionId, ex.exerciseId),
      onUpdate = ForeignKeyAction.Cascade,
      onDelete = ForeignKeyAction.Cascade
    )

    @unused def topicsForeignKey = foreignKey("exercise_topics_topic_fk", (toolId, topicAbbreviation), topicsTQ)(
