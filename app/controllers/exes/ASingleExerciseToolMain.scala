...
import model.{CompleteExWrapper, SingleCompleteEx, User}
...
case class UserExOverviewContent(numOfExes: Int, exesAndRoutes: Seq[ExAndRoute]) {
  ...
  def pages: Int = numOfExes / SingleExerciseController.STEP + 1

}

abstract class ASingleExerciseToolMain(urlPart: String) extends AToolMain(urlPart) {
  ...
  def exerciseRoutes(exercise: CompExType): Seq[(PartType, Call)] = exParts flatMap {
    exPart: PartType =>
      ...
      if (exercise.hasPart(exPart)) {
        ...
        this match {
          case _: AFileExerciseToolMain => Some((exPart, controllers.exes.routes.FileExerciseController.exercise(urlPart, exercise.ex.id, exPart.urlName)))
