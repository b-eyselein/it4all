...
class QuestionController @Inject()(cc: ControllerComponents, dbcp: DatabaseConfigProvider, r: Repository)
  ...
  extends AIdExController[Question, QuestionResult](cc, dbcp, r, QuestionToolObject) with HasDatabaseConfigProvider[JdbcProfile] with Secured {

  override type SolType = StringSolution

  override def solForm: Form[StringSolution] = ???

  override def reads: Reads[Question] = null // QuestionReads.questionReads
  ...
  def quizStart(quizId: Int): EssentialAction = withUser { user => implicit request => Redirect(controllers.questions.routes.QuestionController.quizQuestion(quizId, 1)) }
